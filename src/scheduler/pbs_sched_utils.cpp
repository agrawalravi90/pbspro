...
static int (*schedule_func)(int, const sched_cmd *);
...
restart(int sig)
{
	...
	schedule_func(clust_primary_sock, &cmd);
}
...
int
sched_main(int argc, char *argv[], int (*schedule_func)(int, const sched_cmd *))
{
	int go;
	int c;
	int errflg = 0;
...
schedule_wrapper(sched_cmd *cmd, int opt_no_restart)
{
	...
	if (schedule_func(clust_primary_sock, cmd)) /* magic happens here */
		return 1;
	else
		send_cycle_end();
