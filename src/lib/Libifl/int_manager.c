 ...
 */
static int
PBSD_manager_inner(int c, int function, int command, int objtype, char *objname, struct attropl *aoplp, char *extend)
{
	int rc = 0;
...
PBSD_manager(int c, int function, int command, int objtype, char *objname, struct attropl *aoplp, char *extend)
{
	...
	if ((pbs_verify_attributes(random_srv_conn(svr_conns), function, objtype,
				   command, aoplp)) != 0)
		return pbs_errno;
	...
	if (svr_conns) {
		...
		for (i = start, ct = 0; ct < nsvrs; i = (i + 1) % nsvrs, ct++) {
			...
			*/
			...
			rc = PBSD_manager_inner(svr_conns[i]->sd, function,
						command, objtype, objname,
						aoplp, extend);
	...
	return PBSD_manager_inner(c, function, command, objtype,
				  objname, aoplp, extend);
}
