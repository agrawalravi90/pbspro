...
int
job_save_fs(void *pj, int updatetype)
{
	int fds;
	int i;
	char *filename;
	...
	if (*pjob->ji_qs.ji_fileprefix != '\0')
		...
		strcat(namebuf1, pjob->ji_qs.ji_jobid);
...
job_recov_fs(char *filename)
{
	...
	pj = job_alloc_generic();
	if (pj == NULL) {
		return NULL;
	}

	strcpy(pbs_recov_filename, path_jobs);	/* job directory path */
	strcat(pbs_recov_filename, filename);
#ifdef WIN32
	...
	strcpy(basen, pbs_recov_filename);
	...
	secure_file(pbs_recov_filename, "Administrators",
		READS_MASK|WRITES_MASK|STANDARD_RIGHTS_REQUIRED);
#else
	rename(basen, pbs_recov_filename);
#endif
...
job_or_resv_save_fs(void *pobj, int updatetype, int objtype)
{
	...
	strcpy(namebuf1, path);		/* directory path */
	strcat(namebuf1, prefix);
	strcpy(namebuf2, namebuf1);	/* setup for later */
	strcat(namebuf1, suffix);
...
job_or_resv_recov_fs(char *filename, int objtype)
{
	...
	strcpy(namebuf, path);	/* job (reservation) directory path */
	...
	if (recov_attr_fs(fds, pobj, p_attr_def, wattr,
		...
		if (objtype == RESC_RESV_OBJECT) {
			...
			job_free_generic((svrjob_t *) pobj);
		}
