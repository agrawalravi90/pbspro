...
mom_comm(job *pjob, void (*func)(struct work_task *))
{
	...
	if (pjob->ji_momhandle < 0) {
		...
		if (pjob->ji_qs.ji_un.ji_exect.ji_momaddr == 0) {
			if (!is_jattr_set(pjob, JOB_ATR_exec_vnode) || get_jattr_str(pjob, JOB_ATR_exec_vnode) == NULL)
				return -1;
			pjob->ji_qs.ji_un.ji_exect.ji_momaddr = get_addr_of_nodebyname(
					get_jattr_str(pjob, JOB_ATR_exec_vnode), &dum);
			if (pjob->ji_qs.ji_un.ji_exect.ji_momaddr == 0)
				return -1;
...
unset_extra_attributes(job *pjob)
{
	...
	if (is_jattr_set(pjob,  JOB_ATR_resource_orig)) {
		...
		set_attr_with_attr(&job_attr_def[(int) JOB_ATR_resource], &pjob->ji_wattr[(int) JOB_ATR_resource],
				&pjob->ji_wattr[(int) JOB_ATR_resource_orig], INCR);
