...
static void set_depend_hold(svrjob_t *, attribute *);
...
static void update_depend(svrjob_t *, char *, char *, int, int);
 ...
 * @return	void
 */
static void update_depend(svrjob_t *pjob, char *d_jobid, char *d_svr, int op, int type)
{
	svrjob_t *d_job;
	struct depend *dp;
	struct depend_job *dpj;

	d_job = find_svrjob(d_jobid);
	if (d_job == NULL)
		return;
...
/**
 * @brief
 * 		req_register_dep - process the Register Dependency Request
 * @note
...
void
req_register_dep(struct batch_request *preq)
{
	int made;
	attribute *pattr;
	struct depend *pdep;
	...
	if ((pjob = find_svrjob(preq->rq_ind.rq_register.rq_parent)) == NULL) {
				...
				case JOB_DEPEND_TYPE_RUNONE:
					...
					if (svr_chk_owner(preq, pjob)) {
						...
						if (pdep) {
							...
							svrjob_t *pr_job;
							pr_job = find_svrjob(preq->rq_ind.rq_register.rq_child);
...
post_doq(struct work_task *pwt)
{
	...
	if (preq->rq_reply.brp_code) {
		...
		if (pjob) {
			ppjob = find_svrjob(preq->rq_ind.rq_register.rq_parent);
...
static void
alter_unreg(svrjob_t *pjob, attribute *old, attribute *new)
{
	struct depend	*poldd;
	struct depend	*pnewd;
...
depend_on_que(attribute *pattr, void *pobj, int mode)
{
	...
	while (pdep) {
		...
		if (type !=JOB_DEPEND_TYPE_ON) {
			...
			while (pparent) {
				...
				if (type == JOB_DEPEND_TYPE_RUNONE) {
					svrjob_t *djob = find_svrjob(pparent->dc_child);
					if (djob == NULL)
						return PBSE_IVALREQ;
	...
	if (pdep != NULL) {
		...
		for (; f_pparent != NULL; f_pparent = (struct depend_job *)GET_NEXT(f_pparent->dc_link)) {
			pjob = find_svrjob(f_pparent->dc_child);
			if (pjob == NULL)
				return (0);
...
post_runone(struct work_task *pwt)
{
	...
	if (pjob) {
		...
		if (pdep != NULL) {
			...
			del_job = find_svrjob(preq->rq_ind.rq_register.rq_parent);
 ...
 */
int
depend_runone_remove_dependency(svrjob_t *pjob)
{
	struct depend *pdep;
	struct depend_job *pdj;
	svrjob_t *d_pjob;
	...
	if (pdep) {
		for (pdj = (struct depend_job *)GET_NEXT(pdep->dp_jobs);
			...
			d_pjob = find_svrjob(pdj->dc_child);
			if (d_pjob) {
 ...
 */
int
depend_runone_hold_all(svrjob_t *pjob)
{
	struct depend *pdep;
	struct depend_job *pdj;
	svrjob_t *d_pjob;
 ...
 */
int
depend_runone_release_all(svrjob_t *pjob)
{
	struct depend *pdep;
	struct depend_job *pdj;
	svrjob_t *d_pjob;
...
static void
set_depend_hold(svrjob_t *pjob, attribute *pattr)
{
	int		loop = 1;
	int		newstate;
	int		newsubst;
...
int
send_depend_req(svrjob_t *pjob, struct depend_job *pparent, int type, int op, int schedhint, void (*postfunc)(struct work_task *))
{
	int		      i;
	char		     *pc;
